...
fastify.register(async function (fastify) {
	...
	fastify.get<{ Params: { gameId: string } }>(
		...
		{ websocket: true },
		(socket, reply) => {
			const gameId = reply.params.gameId;
			...
			if (!room) {
				...
				setTimeout(() => socket.close(1008, "Invalid game ID"), 100); // 1008 = Policy Violation
				return;
			}

			room.sockets.push(socket);
			console.log(`Player joined room ${gameId} (${room.sockets.length}/2)`);
			socket.send(`You joined game ${gameId}. Waiting for another player...`);

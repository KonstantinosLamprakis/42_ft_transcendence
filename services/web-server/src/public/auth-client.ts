...
(window as any).getProfile = async () => {
	...
	let imgPath: string = "";
	if (data.avatar) {
		imgPath = !data.isGoogleAccount ? `${HTTPS_API_URL}/uploads/${data.avatar}` : data.avatar;
	}

    const disable2FA = data.isGoogleAccount ? "disabled title='2FA is not available for Google accounts'" : "";
    ...
    <button onclick="setup2FA('${data.username}')" class="btn-secondary mt-2" ${disable2FA}>Enable 2FA</button>
  `;
};
...
console.log(`${HTTPS_API_URL}/google-login`);

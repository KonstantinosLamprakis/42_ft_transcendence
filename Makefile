...
BACKEND_SERVICES := $(shell find backend -maxdepth 1 -mindepth 1 -type d) # List all directories in backend automatically
...
build: install
	...
	cd $(REPO_ROOT)backend/web-server && npm run build:frontend
...
clean:
	...
	rm -rf $(REPO_ROOT)backend/web-server/public/*.js
